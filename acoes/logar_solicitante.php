<?php
    ...
    if(isset($_POST['email']) && isset($_POST['senha']) && $conexao != null){ //só entra se existir email, senha e a conexão estiver certinha, ent só usuário que colocou os dados no login consegue acessar essa parte
        $query = $conexao->prepare("SELECT * FROM Solicitante WHERE email = ? AND senha = ?"); // procura td de usuário no banco | ? = atribuir valor quando executar
        $query->execute(array($_POST["email"], $_POST["senha"])); //executa | atribui valores que colocamos como ? = 1:email que foi recebido 2:senha que foi recebida
        if ($query->rowCount()){ //row = resultados do select | Count = conta os resultados | if espera true (1), ent nn precisa colocar o 1
            ...
            $_SESSION["usuario"] = array($user['nome_Solicitante'], $user['id']); // nome da sessão vai ser usuário e vai ter um array com nome de usuário e o adm
